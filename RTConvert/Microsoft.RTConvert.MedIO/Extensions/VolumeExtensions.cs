...
namespace Microsoft.RTConvert.MedIO.Extensions
{
    ...
    public static class VolumeExtensions
    {
        ...
        /// <summary>
        /// Convert a Volume2D to a Volume3D by reproducing it along z axis.
        /// </summary>
        /// <typeparam name="T">Volume data type.</typeparam>
        /// <param name="volume2D">Source volume2D.</param>
        /// <param name="dimZ">Target dimZ.</param>
        /// <param name="spacingZ">Target spacingZ.</param>
        /// <returns>Volume3D consisting of dimZ copies of volume2D.</returns>
        public static Volume3D<T> Extrude<T>(this Volume2D<T> volume2D, int dimZ, double spacingZ = 1.0)
        {
            var volume3D = new T[volume2D.Length * dimZ];

            for (int z = 0; z < dimZ; z++)
            {
                var startIndex = z * volume2D.DimXY;

                for (int i = 0; i < volume2D.Length; i++)
                {
                    volume3D[startIndex + i] = volume2D[i];
                }
            }

            return new Volume3D<T>(volume3D,
                volume2D.DimX, volume2D.DimY, dimZ,
                volume2D.SpacingX, volume2D.SpacingY, spacingZ);
        }
