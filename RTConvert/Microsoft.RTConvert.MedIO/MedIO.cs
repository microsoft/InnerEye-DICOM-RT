...
namespace Microsoft.RTConvert.MedIO
{
    ...
    using Writers;
    using static NiftiIO.NiftiInternal;
    ...
    public class MedIO
    {
        ...
        /// <summary>
        /// Loads a Nifti stream, returning it as a <see cref="Volume3D{T}"/> with datatype
        /// <see cref="byte"/>, irrespective of the datatype used in the Nifti file itself.
        /// </summary>
        /// <param name="MemoryStream">Memory stream to load.</param>
        /// <returns></returns>
        public static Volume3D<byte> LoadNiftiAsByte(MemoryStream stream)
        {
            return LoadNiftiFromStream(stream, NiftiIO.ReadNiftiAsByte);
        }
        ...
        /// <summary>
        /// Save a 3D volume to a memory stream, in compressed or uncompressed Nifti format.
        /// </summary>
        /// <param name="image"></param>
        public static MemoryStream SaveNiftiToStream(Volume3D<short> image)
        {
           return SaveNiftiToStream(image, NiftiIO.WriteToStream);
        }
        ...
        private static T LoadNiftiFromStream<T>(MemoryStream stream, Func<Stream, NiftiCompression, T> loadFromStream)
        {
            // We are assuming GZip compression here
            return loadFromStream(stream, NiftiCompression.GZip);
        }
        ...
        private static MemoryStream SaveNiftiToStream<T>(T volume, Action<Stream, T, NiftiCompression> saveToStream)
        {
            try
            {
                var memoryStream = new MemoryStream();
                saveToStream(memoryStream, volume, NiftiCompression.GZip);
                return memoryStream;
            }
            catch (Exception ex)
            {
                throw new Exception($"Error writing to memory stream: {ex.Message}", ex);
            }
        }
